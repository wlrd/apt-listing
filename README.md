...
2. Upload everything inside this folder—`index.html`, `styles.css`, `script.js`, `robots.txt`, `sitemap.xml`, `.nojekyll`, and the `images` folder—to the root of the repository.
...
The inquiry form does not collect or store data. It creates a pre-filled email in the visitor's own email app. Search and sharing metadata, structured apartment details, and absolute social-image URLs are configured for the live GitHub Pages address.
